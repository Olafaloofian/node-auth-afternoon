...
app.get('/callback', (req, res) => {
  exchangeCodeForAccessToken()
    ...
    .catch(error => {console.log('------------ error', error); res.status(500).send('An error occurred on the server, check the server terminal')})
  ...
  function exchangeCodeForAccessToken() {
    ...
    function setGitTokenToSession (gitHubAccessTokenResponse) {
      console.log('------------ gitHubAccessTokenResponse.data.identities[0].access_token', gitHubAccessTokenResponse.data.identities[0].access_token)
...
app.put('/api/star', (req, res) => {
  ...
  axios.put(`https://api.github.com/user/starred/${gitUser}/${gitRepo}?access_token=${req.session.gitHubAccessToken}`).then( () => res.end()).catch( error => console.log('------------ error', error))
})

app.delete('/api/star', (req, res) => {
  const { gitUser, gitRepo } = req.query
  axios.delete(`https://api.github.com/user/starred/${gitUser}/${gitRepo}?access_token=${req.session.gitHubAccessToken}`).then( () => res.end()).catch( error => console.log('------------ error', error))
})
